...
class CustomNotebook(ttk.Notebook):
    ...
    def add_py_tab(self, event=None, _file=None, _file_name=None):
        ...
        fn = f'Py File {len(self.py_files)+1}'
    ...
    def add_console_tab(self, event=None):
        ...
        tab = TextConsole(tab_frame, locals(), self.master.destroy)
    ...
    def select_new_tab(self, event=None):
        ...
        sf_root.geometry(f'200x130+{int(sf_root.winfo_screenheight() * 0.75)}+'
                         f'{int(sf_root.winfo_screenwidth() * 0.16)}')
        sf_root.overrideredirect(True)
        _offsetx = 0
        _offsety = 0
        sf_root.bind('<Button-1>', clickwin)
        sf_root.bind('<B1-Motion>', dragwin)

        exit_lbl = Label(sf_root, text='×', font=('Consolas 12'), anchor=E, cursor='hand2', bg='gray84')
        exit_lbl.pack(side=TOP, anchor=N+E, expand=YES)
        exit_lbl.bind('<Button-1>', lambda a: sf_root.destroy())
        exit_lbl.bind('<Enter>', lambda a: exit_lbl.config(fg='orange'))
        exit_lbl.bind('<Leave>', lambda a: exit_lbl.config(fg='black'))

        new_lbl = Label(sf_root, text='+ Py File', font=('Consolas 13'), bg='gray84', cursor='hand2')
        new_lbl.pack(side=TOP)
        new_lbl.bind('<Button-1>', self.add_py_tab)
        new_lbl.bind('<Enter>', lambda a: new_lbl.config(fg='orange'))
        new_lbl.bind('<Leave>', lambda a: new_lbl.config(fg='black'))

        console_lbl = Label(sf_root, text='+ New Console', font=('Consolas', 13), bg='gray84', cursor='hand2')
        console_lbl.pack(side=TOP)
        console_lbl.bind('<Button-1>', self.add_console_tab)
        console_lbl.bind('<Enter>', lambda a: console_lbl.config(fg='orange'))
        console_lbl.bind('<Leave>', lambda a: console_lbl.config(fg='black'))

        open_lbl = Label(sf_root, text='+ Text File', font=('Consolas 13'), bg='gray84', cursor='hand2')
        open_lbl.pack(side=TOP)
        open_lbl.bind('<Button-1>', self.add_file_tab)
        open_lbl.bind('<Enter>', lambda a: open_lbl.config(fg='orange'))
        open_lbl.bind('<Leave>', lambda a: open_lbl.config(fg='black'))

        open_md_lbl = Label(sf_root, text='+ README.md', font='Consolas 13', bg='gray84', cursor='hand2')
        open_md_lbl.pack(side=TOP)
        open_md_lbl.bind('<Button-1>', self.add_md)
        open_md_lbl.bind('<Enter>', lambda a: open_md_lbl.config(fg='orange'))
        open_md_lbl.bind('<Leave>', lambda a: open_md_lbl.config(fg='black'))

        sf_root.bind('<Escape>', lambda a: sf_root.destroy())
...
class ColorTextTab(Frame):
    ...
    def __init__(self, **kwargs):
        ...
        self.colortext = ColorText(master=self, wrap=None, yscrollcommand=v_scroll.set, xscrollcommand=h_scroll.set,
                                   undo=True)
...
if __name__ == '__main__':
    ...
    root.bind('<Control-N>', ez_py.add_py_tab)
    root.bind('<Control-n>', ez_py.add_py_tab)
    root.bind('<Control-Shift-C>', ez_py.add_console_tab)
    root.bind('<Control-Shift-c>', ez_py.add_console_tab)
    root.bind('<Control-Shift-N>', ez_py.select_new_tab)
    root.bind('<Control-Shift-n>', ez_py.select_new_tab)
    ...
    themes.bind('<Button-1>', theme_settings)
    ez_py.focus_force()
    root.mainloop()
